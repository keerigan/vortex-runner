...
func _make_world() -> void:
	super._make_world()
	# Portrait framing: keep a hint of cinematic offset without pushing the fighter off-screen.
	camera.position = Vector3(0.12, -0.42, 8.05)
	camera.rotation_degrees = Vector3(-2.0, -0.8, -0.15)
	camera.fov = 64.0
	var fill := OmniLight3D.new()
	fill.position = Vector3(-2.7, -0.9, 1.4)
	fill.light_color = Color(0.58, 0.16, 0.90)
...
func _build_corridor_section(section: Node3D, index: int) -> void:
	...
	var pale := _mat(Color(0.58, 0.56, 0.64), Color(0.05, 0.025, 0.08), 0.22, 0.50, 0.28)
	var floor_glow := _mat(Color(0.34, 0.27, 0.42), Color(0.30, 0.08, 0.42), 0.75, 0.36, 0.24)
	...
	# Large illuminated deck surfaces hide much of the arcade-like dotted runway below.
	_box(section, Vector3(-2.55, -2.89, 0.0), Vector3(3.55, 0.09, 6.35), floor_glow)
	_box(section, Vector3(2.55, -2.89, 0.0), Vector3(3.55, 0.09, 6.35), floor_glow)
	_box(section, Vector3(0.0, -2.84, 0.0), Vector3(1.25, 0.07, 6.45), charcoal)
	# Long seams add scale without turning the floor into a light-show grid.
	for x: float in [-4.05, -1.10, 1.10, 4.05]:
		_box(section, Vector3(x, -2.81, 0.0), Vector3(0.035, 0.018, 6.0), steel)

	# Broad ceiling ribs with service channels between them.
	...
	# Sparse warm guidance lamps.
	if index % 3 == 0:
		_box(section, Vector3(-3.15, -2.77, -1.65), Vector3(0.44, 0.045, 0.13), amber)
	if index % 3 == 1:
		_box(section, Vector3(3.15, -2.77, 1.25), Vector3(0.44, 0.045, 0.13), amber)

	# Actual light pools on selected modules and floor.
	if index % 4 == 0:
...
func _update_camera(delta: float) -> void:
	var target := Vector3(0.12 + ship.position.x * 0.025, -0.42 + ship.position.y * 0.012, 8.05)
	camera.position = camera.position.lerp(target, clampf(delta * 1.6, 0.0, 1.0))
	camera.fov = lerpf(camera.fov, 64.0 + (speed - 13.0) * 0.07, clampf(delta * 1.3, 0.0, 1.0))
		...
		if child is CanvasLayer:
			for control in child.get_children():
				if control is Label and control.text.begins_with("VORTEX // RUNNER"):
					control.text = "VORTEX // RUNNER 1.1 // REVIEW"
