...
func _make_world() -> void:
	super._make_world()
	# Slight upward pitch keeps the portrait frame filled with architecture instead of empty black.
	camera.position = Vector3(0.08, -0.34, 8.00)
	camera.rotation_degrees = Vector3(2.8, -0.45, -0.10)
	camera.fov = 64.0
	var cool_fill := OmniLight3D.new()
	cool_fill.position = Vector3(-2.8, -0.6, 1.4)
	cool_fill.light_color = Color(0.18, 0.52, 0.95)
	cool_fill.light_energy = 3.6
...
func _build_corridor_section(section: Node3D, index: int) -> void:
	...
	var cyan := _mat(Color(0.14, 0.91, 1.0), Color(0.0, 0.78, 1.0), 5.3, 0.14, 0.06)
	var violet := _mat(Color(0.43, 0.15, 0.58), Color(0.54, 0.07, 0.78), 2.2, 0.20, 0.10)
	var amber := _mat(Color(0.94, 0.34, 0.06), Color(1.0, 0.15, 0.01), 3.8, 0.15, 0.08)

	# Wide deck planes and central dark trench.
	...
	# Thick walls, angled armor and service ledges.
	for side: float in [-1.0, 1.0]:
		...
		# Industrial pod bank, intentionally large enough to read from gameplay distance.
		for k in range(3):
		...
		# One continuous signature strip, not a dotted arcade rail.
		_box(section, Vector3(side * 4.43, 0.66, 0.0), Vector3(0.09, 0.095, SECTION_LENGTH * 0.96), cyan)

	# Layered ceiling with visible beams close to the camera.
	...
	# Structural portals create a strong room-like silhouette.
	if index % 2 == 0:
	...
	# Large authored landmarks alternate sides to stop the corridor reading as mirrored wallpaper.
	var featured_side := -1.0 if index % 6 < 3 else 1.0
	if index % 5 == 0:
		_cylinder(section, Vector3(featured_side * 3.28, -0.42, -0.72), 0.94, 0.34, gunmetal, Vector3(0.0, 0.0, 90.0))
		_cylinder(section, Vector3(featured_side * 3.06, -0.42, -0.72), 0.70, 0.38, pale, Vector3(0.0, 0.0, 90.0))
		_cylinder(section, Vector3(featured_side * 2.83, -0.42, -0.72), 0.43, 0.42, cyan, Vector3(0.0, 0.0, 90.0))
		_cylinder(section, Vector3(featured_side * 2.59, -0.42, -0.72), 0.19, 0.46, dark, Vector3(0.0, 0.0, 90.0))
	if index % 5 == 2:
...
func _update_camera(delta: float) -> void:
	var target := Vector3(0.08 + ship.position.x * 0.020, -0.34 + ship.position.y * 0.010, 8.00)
	camera.position = camera.position.lerp(target, clampf(delta * 1.5, 0.0, 1.0))
	camera.fov = lerpf(camera.fov, 64.0 + (speed - 13.0) * 0.07, clampf(delta * 1.2, 0.0, 1.0))
		...
		if child is CanvasLayer:
			for control in child.get_children():
				if control is Label and control.text.begins_with("VORTEX // RUNNER"):
					control.text = "VORTEX // RUNNER 1.3 // REVIEW"
