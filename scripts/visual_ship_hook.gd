...
func _make_world() -> void:
	super._make_world()
	# Balanced portrait framing: enough ceiling to feel enclosed, without sacrificing half the screen.
	camera.position = Vector3(0.08, -0.38, 8.00)
	camera.rotation_degrees = Vector3(0.2, -0.45, -0.10)
	camera.fov = 65.0
...
func _build_corridor_section(section: Node3D, index: int) -> void:
	...
	var cyan := _mat(Color(0.14, 0.91, 1.0), Color(0.0, 0.78, 1.0), 4.5, 0.14, 0.06)
	...
	# Ceiling with broad plates, transverse ribs and cable runs.
	...
	if index % 5 == 0:
		...
		# Braced housing around the large turbine makes it feel attached to the environment.
		...
		if child is CanvasLayer:
			for control in child.get_children():
				if control is Label and control.text.begins_with("VORTEX // RUNNER"):
					control.text = "VORTEX // RUNNER 1.4 // REVIEW"
